...
async function flush(data) {
  ...
  const prods = data.products.map((p, i) => ({
    ...
    preorder: !!p.preorder, custom: !!p.custom, private: !!p.private,
  }));
  if (prods.length) {
...
export default function AdminClient({ initialData, userEmail }) {
  ...
  const addProduct = () =>
    setData((d) => ({
      ...d,
      products: [
        { id: uid(), name: "New listing", cat: d.categories[0]?.name || "Everything", price: "$0", tone: "t2", blurb: "", desc: "", images: [], sizes: [], specs: [], featured: false, available: true, preorder: false, custom: false, stock: [], private: false },
        ...d.products,
      ],
    }));
  const addPrivateProduct = () => {
    ...
    setData((d) => ({
      ...d,
      products: [
        { id, name: "Custom order", cat: d.categories[0]?.name || "Everything", price: "let's talk", tone: "t3", blurb: "", desc: "", images: [], sizes: [], specs: [], featured: false, available: true, preorder: false, custom: false, stock: [], private: true },
        ...d.products,
      ],
    }));
  };
              ...
              <div className="adm-item-fields">
                ...
                <div className="adm-item-foot">
                  <div className="adm-foot-checks">
                    ...
                    <label className="adm-check"><input type="checkbox" checked={!!p.custom} onChange={(e) => patchProduct(p.id, { custom: e.target.checked })} /> custom design (rainbow tag)</label>
